...
from books.dtos.CategoryDto import CategoryDto
# Import dto
from books.dtos.UserDto import UserDto
from books.dtos.AdminDto import AdminDto
from books.dtos.PhieunhapDto import PhieunhapDto
from books.dtos.BookDto import BookDto
from books.dtos.DocgiaDto import DocgiaDto
from books.dtos.ThethuvienDto import ThethuvienDto
from books.dtos.KiemkeDto import KiemkeDto
from books.dtos.CTKiemkeDto import CTKiemkeDto

from books.dtos.PhieumuonDto import PhieumuonDto
from books.dtos.PhieuhuyDto import PhieuhuyDto
...
def device_list(request):
    ...
    return render(request, 'thietbi/device_list.html', {'devices': devices})

def device_detail(request, pk):
    device = get_object_or_404(Device, pk=pk)
    return render(request, 'thietbi/device_detail.html', {'device': device})
...
def device_create(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return redirect('device_list')  # Redirect to device list after successful creation
    else:
        form = DeviceForm()
    return render(request, 'thietbi/device_form.html', {'form': form})
...
def device_update(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            device = form.save()
            return redirect('device_list')  # Redirect to home page after successful update
    else:
        form = DeviceForm(instance=device)
        form.fields['purchase_date'].initial = device.purchase_date.strftime('%Y-%m-%d')
    return render(request, 'thietbi/device_form.html', {'form': form})
...
def searchDevice(request, searchInput):
    devices = Device.objects.filter(name__icontains=searchInput)
    return render(request, 'thietbi/device_list.html', {'devices': devices})